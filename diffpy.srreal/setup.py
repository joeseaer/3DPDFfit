...
# >>> ADD THIS BLOCK <<<
import os
DIFFPY_LIBDIFFPY_BUILD = os.environ.get("DIFFPY_LIBDIFFPY_BUILD")
if not DIFFPY_LIBDIFFPY_BUILD:
    raise EnvironmentError(
        "Please set DIFFPY_LIBDIFFPY_BUILD to the build directory of diffpy.libdiffpy, "
        "e.g., ~/workspace/diffpy.libdiffpy/build/fast-x86_64"
    )
diffpy_include = os.path.join(os.path.dirname(DIFFPY_LIBDIFFPY_BUILD), "..", "src")
# <<< END ADD <<<
...
ext_kws = {
    ...
    ] + boost_cfg["include_dirs"],
    "library_dirs": [
        DIFFPY_LIBDIFFPY_BUILD,       # 鈫?鍔犺繖涓€琛岋紒
    ] + boost_cfg["library_dirs"],
...
if __name__ == "__main__":
    setup(
        name="diffpy.srreal",
        package_dir={"": "src"},                     # 鈫?鍏抽敭锛氭簮鐮佸湪 src/
        packages=find_packages(where="src"),         # 鈫?鍏抽敭锛氳嚜鍔ㄥ彂鐜?src/ 涓嬬殑鍖?        ext_modules=ext_modules(),                   # 鈫?浣犵殑鎵╁睍妯″潡
        python_requires=">=3.7")

...
PDF3DCPUCalculator -- scalar CPU 3D PDF calculator
PDF3DGPUCalculator -- calculate 3D real-space PDF with optional CUDA acceleration
"""
...
def _defineCommonInterface(cls):
    ...
    def _call_kwargs(self, structure=None, **kwargs):
        ...
        ----------
        structure
            a structure object to be evaluated. Reuse the last structure when None.
        kwargs
...
# PDF3DCPUCalculator --------------------------------------------------------
_defineCommonInterface(PDF3DCPUCalculator)


def _init_kwargs_cpu(self, **kwargs):
    """Create a new instance of PDF3DCPUCalculator.

    Keyword arguments can configure calculator properties, including
    cpuworkfactor for controlling scalar CPU reference work.
    """
    PDF3DCPUCalculator.__boostpython__init(self)
    setattrFromKeywordArguments(self, **kwargs)
    return


PDF3DCPUCalculator.__boostpython__init = PDF3DCPUCalculator.__init__
PDF3DCPUCalculator.__init__ = _init_kwargs_cpu

# End of class PDF3DCPUCalculator


# PDF3DGPUCalculator ---------------------------------------------------------
_defineCommonInterface(PDF3DGPUCalculator)


def _init_kwargs3(self, **kwargs):
    """Create a new instance of PDF3DGPUCalculator.

    Keyword arguments can be used to configure the calculator properties,
    for example:
        p3c = PDF3DGPUCalculator(rmax=10.0, gridstep=0.1, qmax=12)

    Raise ValueError for invalid keyword argument.
    """
    PDF3DGPUCalculator.__boostpython__init(self)
    setattrFromKeywordArguments(self, **kwargs)
    return


PDF3DGPUCalculator.__boostpython__init = PDF3DGPUCalculator.__init__
PDF3DGPUCalculator.__init__ = _init_kwargs3

# End of class PDF3DGPUCalculator
